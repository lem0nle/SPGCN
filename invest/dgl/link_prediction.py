...
def run(proc_id, n_gpus, args, devices, data):
    ...
    train_graph, valid_graph, test_graph, test_neg_graph = data
    th.random.manual_seed(523)
    nfeat = th.rand(train_graph.num_nodes(), 100)
    ...
    dataloader = dgl.dataloading.EdgeDataLoader(
        train_graph, train_seeds, sampler, exclude='reverse_id',
        # For each edge with ID e in Reddit dataset, the reverse edge is e ± |E|/2.
        reverse_eids=th.cat([
            th.arange(n_edges // 2, n_edges),
            th.arange(0, n_edges // 2)]).to(train_seeds),
        negative_sampler=NegativeSampler(train_graph, args.num_negs, args.neg_share),
        device=device,
        # use_ddp=n_gpus > 1,
...
def main(args, devices):
    ...
    data = train_graph, valid_graph, test_graph, test_neg_graph
